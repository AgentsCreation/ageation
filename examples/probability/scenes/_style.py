...
Conventions (matching the source notes in input/Probability/, see NOTATION.md):
- Probability ``\\Pr(...)``, expectation ``\\mathrm{E}[...]``, variance
  ``\\mathrm{Var}(...)`` -- use the pr()/expectation()/variance() helpers, which
  emit the expanded forms (Manim has no access to the book's LaTeX macros).
...
def pr(expr: str) -> str:
    r"""Return a LaTeX probability expression using \Pr, matching the notes.
...
def expectation(expr: str) -> str:
    r"""Expectation in the project convention: ``\mathrm{E}\left[expr\right]``.

    Matches the notes' ``\Expect`` macro (not blackboard-bold E). See NOTATION.md.
    """
    return r"\mathrm{E}\left[" + expr + r"\right]"


def variance(expr: str) -> str:
    r"""Variance in the project convention: ``\mathrm{Var}\left(expr\right)``.

    Matches the notes' ``\Var`` macro. See NOTATION.md.
    """
