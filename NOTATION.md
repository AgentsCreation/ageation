...
A course's mathematical notation must agree across three places: the source
...
In the project's `course.yaml`, under `notation.rules`. Each rule is a literal
...
document — is `examples/probability/` (`course.yaml` + `NOTATION.md` there).
...
choices not pinned by any macro. Keep it next to `course.yaml` and update both
