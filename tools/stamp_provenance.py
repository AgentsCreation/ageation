...
from provenance import (sha256_file, sha256_script, split_fm, rebuild, fm_get,
                        fm_upsert, framework_provenance)
from _project import project_parser, resolve_project, yaml_scalar

DATE = datetime.date.today().isoformat()

SCENE_SHA_RE = re.compile(r"(?m)^# derived_from_sha256:.*$")

# Resolve the engine version ONCE, at import — before any stamping. Stamping an
# in-repo project (e.g. examples/probability) mutates the engine tree, so a
# recompute at the end of the run would spuriously report `-dirty` and disagree
# with the per-concept `framework_commit` stamps below. Both read this snapshot.
FRAMEWORK_PROV = framework_provenance()
FRAMEWORK = FRAMEWORK_PROV["version"]


def write_build_yaml(root, built):
    """Write BUILD.yaml — the project's canonical record of its engine version.

    Generated, not hand-authored: one top-level summary of which ageation
    checkout last built this project, alongside the per-concept
    `framework_commit` stamps (which serve file-level drift detection). Uses
    the import-time FRAMEWORK_PROV so it agrees with those stamps even when the
    stamp run itself dirties an in-repo project's tree.
    """
    prov = FRAMEWORK_PROV
    path = os.path.join(root, "BUILD.yaml")
    lines = [
        "# Generated by ageation -- do not edit by hand.",
        "# Records the engine version that last built this project.",
        "# Refresh with: python tools/stamp_provenance.py --project .",
        "engine:",
        f"  version: {yaml_scalar(prov['version'])}",
        f"  commit: {yaml_scalar(prov['commit'])}",
        f"  package_version: {yaml_scalar(prov['package_version'])}",
        f"  dirty: {'true' if prov['dirty'] else 'false'}",
        f"built: {yaml_scalar(built)}",
        "",
    ]
...
def main():
    ...
    path, prov = write_build_yaml(root, DATE)
    print(f"  wrote {os.path.basename(path)}  (engine {prov['version']})")
