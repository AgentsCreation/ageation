...
| Architecture (2026-06-11, after the nano-evolve case study) | **engine/content split**: this repo is the engine; projects live outside it, preferably as a reserved `ageation/` subdirectory of the base repo (`upstream_dir` pointing back into the host; no `input/`), or standalone with `input/`. The manifest is **`project.yaml`** (renamed from course.yaml — the framework is broader than courses); `stamp_provenance.py` records `framework_commit` per concept. | One reserved name (not five prefixed ones) keeps the namespace disjoint from any host layout (e.g. the canonical article template); `--project DIR` already made the tools location-agnostic, so the posture cost zero code. Version coupling is detectable via `framework_commit`. |
| The register + layout system (2026-07-04, after the 43-video probability review cycle) | **ageation optimizes for structured/robust over artisanal** (STYLE_BOOK §0). The recurring reviewer notes became code: layout helpers `two_column`/`eq_chain`/`even_stack`/`caption_under`/`chart_tag` in `_style.py`, and `tools/lint_language.py` (in `make check`, warning-first; `--strict` gates) enforcing: no video-number cross-references (topics only — numbering breaks on insertion), no bare "bell"/"ramp" terminology, sentence ≤ ~40 words / voiceover block ≤ ~70 words advisories. | Across ~20 review rounds the same five note categories repeated: graphic position, page balance, font size consistency, over-long sentences, number-brittle callbacks. Each is now either a helper (right by default) or a lint (caught before the human sees the draft). The meta-rule stands: a repeated review note must become shared code, not a per-scene patch. |
| Chapter numbering (2026-07-06, decided before the probability consumer's first commit) | Numeric slug prefixes are **zero-padded, permanent accession IDs** — order lives in the `project.yaml` spine, never in the numbers; insertions take the next free number; numbers never appear in narration or on screen (PIPELINE.md "Chapter numbering"; `slugify` in `init_project.py` pads). | Unpadded prefixes scrambled every listing (1, 10, 11, …, 2); treating numbers as positions would make a mid-course insertion cascade through ~90 filenames, provenance fields, and published deliverable names. ID semantics keep the handles humans need (review notes, upload order) with zero rename pressure. |
